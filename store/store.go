...
func (s *Store) Exit(nodeID, addr string) error {
	s.logger.Printf("received exit request for remote node %s at %s", nodeID, addr)
	...
	for _, srv := range configFuture.Configuration().Servers {
		...
		// If a node already exists with the same exiting node's ID and address,
		// then remove it.
		if srv.Address == raft.ServerAddress(addr) && srv.ID == raft.ServerID(nodeID) {
			s.logger.Printf("node %s at %s is a member of cluster, removing", nodeID, addr)
			future := s.raft.RemoveServer(srv.ID, 0, 0)
			if err := future.Error(); err != nil {
				return fmt.Errorf("error removing existing node %s at %s: %s", nodeID, addr, err)
			}
